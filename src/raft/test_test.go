...
func TestReElection2A(t *testing.T) {
	...
	cfg.checkOneLeader()
	DPrintf("[Test] disconnect leader1 and check one leader")

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("[Test] connect leader1 and check one leader")
	...
	cfg.checkOneLeader()
	DPrintf("[Test] reconnect one server and check one leader")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	DPrintf("[Test] reconnect leader2 and check one leader")
